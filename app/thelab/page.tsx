...
// Heatmap background component
export const HeatmapBackground = () => {
  const gradRef = useRef<HTMLCanvasElement | null>(null);
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  // Trail de posiciones
  const trail = useRef<{x:number,y:number}[]>([]);
  // Paletas de colores para el gradiente
  const palettes = [
    // Paleta 1: azul, violeta, celeste
    [200, 270, 190],
    // Paleta 2: verde, naranja, rosa
    [140, 40, 320],
    // Paleta 3: amarillo, rojo, teal
    [51, 0, 168],
    // Paleta 4: violeta, celeste, rosa
    [270, 190, 320],
    // Paleta 5: naranja, azul, verde
    [40, 200, 140],
  ];
  const [paletteIdx, setPaletteIdx] = React.useState(0);
  // Color dinámico para el trail/círculo
  const [trailColorIdx, setTrailColorIdx] = React.useState(0);
  const trailColorsArr = [
    'rgba(59,130,246,0.22)', // azul
    'rgba(34,197,94,0.22)',  // verde
    'rgba(236,72,153,0.22)', // rosa
    'rgba(251,191,36,0.22)', // amarillo
    'rgba(16,185,129,0.22)', // teal
    'rgba(244,63,94,0.22)',  // rojo
    'rgba(139,92,246,0.22)', // violeta
    'rgba(20,184,166,0.22)', // celeste
    'rgba(253,186,116,0.22)', // naranja
  ];
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let i = 0; i < trail.current.length; i++) {
        ...
        const r = 60 + i*10;
        // Color dinámico para el círculo principal, el resto más transparente
